// src/components/sections/ChatPanel.tsx
...
type Msg = {
  ...
  text: string;
  time: string;
  mode: Mode; // which thread this message belongs to
};

export default function ChatPanel({ character }: ChatPanelProps) {
  ...
  // Visible thread (retain the other tab's messages)
  const thread = messages.filter((m) => m.mode === mode);

  // Auto-scroll when switching tabs or when thread grows
  useEffect(() => {
  ...
  const send = () => {
    ...
    setTimeout(() => {
      ...
      const botMsg: Msg = {
        id: crypto.randomUUID(),
        role: "assistant",
        text: botMsgTextClean(assistantText, character?.name),
        time: stamp(),
        mode,
      };
  ...
  function botMsgTextClean(text: string, name?: string) {
    if (!name) return text.replace(/^\((.*?)\)\s*/, "");
    const exact = new RegExp(`^\\(${name}\\)\\s*`);
    return text.replace(exact, "");
  }

    function cleanAssistantText(text: string) {
    // remove "(Something) " at the start, e.g., (Narrator) or (Albert Einstein)
    return text.replace(/^\([^)]*\)\s*/, "");
    }

    function handleDownload() {
        // only the visible thread
        const thread = messages.filter((m) => m.mode === mode);
    ...
    <Card className="p-4 flex flex-col min-h-[520px] h-full">
      ...
      <div className="flex items-center justify-between px-1 py-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Reset: clear current and switch back to In-Character */}
          <IconButton
            label="Reset"
            onClick={() => {
              setMessages((ms) => ms.filter((m) => m.mode !== mode));
      ...
      {!hasCharacter ? (
        ...
        <>
          ...
          {/* Messages area (only current thread) */}
          <div
            ref={listRef}
            ...
            {thread.length === 0 ? (
                <div className="
                    ...
                    {mode === "in" ? (
                        ...
                        <span className="font-medium text-ink">{character!.name}</span>!
                        {" "}They’ll respond in character.
                        </>
                    ) : (
              ...
              thread.map((m) => {
                ...
                const senderName =
                  m.role === "assistant" ? (m.mode === "in" ? character!.name : "Narrator") : "";
                ...
                return (
                    ...
                    {!isUser && (
                        ...
                        <div className="max-w-[75%]">
                            {/* Show sender name ONLY for in-character; narrator has no label */}
                            {m.mode === "in" && (
                    ...
                    {isUser && (
                        ...
                        <div className="max-w-[70%]">
                            ...
                            <div className="mt-1 text-[11px] text-white/75 text-right">{m.time}</div>
                            </div>
                        </div>
          ...
          <div className="mt-3">
            <div className="relative">
              <input
                ...
                placeholder={mode === "in" ? `Message ${character!.name}...` : `Ask about ${character!.name}...`}
